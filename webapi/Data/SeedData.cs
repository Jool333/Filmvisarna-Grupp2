...
namespace webapi.Data
{
    public static class SeedData
    {
        ...
        public static async Task LoadTicketTypeData(FilmvisarnaContext context)
        {
            var options = new JsonSerializerOptions
            {
                PropertyNameCaseInsensitive = true
            ...
            if (context.TicketType.Any()) return;
            var json = System.IO.File.ReadAllText("Data/json/tickettypes.json");

            var tickets = JsonSerializer.Deserialize<List<TicketType>>(json, options);

            if (tickets is not null && tickets.Count > 0)
            {
                await context.TicketType.AddRangeAsync(tickets);
                await context.SaveChangesAsync();
            }
        }
